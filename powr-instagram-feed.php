<?php
    ...
    Plugin URI: https://www.powr.io/plugins/instagram-feed
    Description: Get more Instagram followers with an embedded gallery  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    class Powr_Instagram_Feed extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Instagram Feed.
        </p>
        <p>
    ...
    function powr_instagram_feed_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-instagram-feed' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-instagram-feed' label='$label'></div>";
      }else{
      	"<div class='powr-instagram-feed'></div>";
      }
    }
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_instagram_feed_menu' );
    function powr_instagram_feed_menu() {
      add_menu_page( 'POWr Instagram Feed', 'POWr Instagram Feed', 'manage_options', 'powr-instagram-feed-settings', 'powr_instagram_feed_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_instagram_feed_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/instagram-feed';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/instagram-feed';
      }
    ...
    register_activation_hook(__FILE__, 'powr_instagram_feed_plugin_activate');
      add_action('admin_init', 'powr_instagram_feed_plugin_redirect');

      function powr_instagram_feed_plugin_activate() {
      add_option('powr_instagram_feed_plugin_do_activation_redirect', true);
      }
      ...
      function powr_instagram_feed_plugin_redirect() {
      if (get_option('powr_instagram_feed_plugin_do_activation_redirect', false)) {
          delete_option('powr_instagram_feed_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-instagram-feed-settings&' );
          }
       }
      }
